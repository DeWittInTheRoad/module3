...
SELECT * FROM weather WHERE parkcode = 'GNP';
SELECT * FROM park WHERE parkcode = 'GNP';

SELECT COUNT(*), park.parkName, park.parkCode FROM survey_result
...
SELECT COUNT(*) AS count, park.parkName, park.parkCode FROM survey_result
JOIN park ON park.parkCode = survey_result.parkCode
GROUP BY park.parkCode
ORDER BY COUNT DESC, park.parkName ASC;

INSERT INTO survey_result(parkCode, emailAddress, state, activityLevel) VALUES ('CVNP', 'fake@fake.com', 'Ohio', 'inactive');
INSERT INTO survey_result(parkCode, emailAddress, state, activityLevel) VALUES ('GNP', 'fake2@fake.com', 'Ohio', 'inactive');
INSERT INTO survey_result(parkCode, emailAddress, state, activityLevel) VALUES ('CVNP', 'fake3@fake.com', 'Ohio', 'inactive');